...
class RetailCatalogProductDetailContext:
...
def build_retail_catalog_product_detail_context(
    *,
    catalog_product: CatalogProduct,
    language_code: str,
) -> RetailCatalogProductDetailContext:
    ...
    offers = tuple(
        build_retail_offer_viewmodel(offer)
        for offer in catalog_product.offers
    )

    if not offers:
        raise ValueError(
            "Retail catalog product must have at least one offer"
        )

    description = _description(profile=profile)
    ingredients = _ingredients(profile=profile)

    return RetailCatalogProductDetailContext(
        catalog_product=catalog_product,
        product_name=product_name,
        category_label=_category_label(
            profile=profile,
        ),
        image_url=_image_url(profile=profile),
...
def _get_product_profile(*, product) -> ProductProfile | None:
    return getattr(product, "profile", None)
...
def _category_label(
    *,
    ...
    return str(profile.get_category_display())
...
def _image_url(
    *,
    ...
    if profile is None or not profile.image:
        return None
