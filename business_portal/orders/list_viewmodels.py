...
def _build_portal_order_page_row(
    order: Order,
) -> PortalOrderPageRow:
    status = build_order_status_presentation(
        order.status
    )
...
def _build_order_card(
    *,
    ...
    return UiCard(
        tone=status.tone,
        css_class=(
            f"{order_card_css_class(order.status)} "
            "portal-history-card"
        ),
        rows=(
            _build_header_row(
