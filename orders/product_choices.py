...
def build_product_choice_context(*, order: Order | None = None) -> ProductChoiceContext:
    ...
    if order is not None:
        existing_product_ids = set(order.lines.values_list("product_id", flat=True))

    if order is not None and order.status != Order.Status.DRAFT:
        for line in order.lines.all():
            available_units[line.product_id] = (
                available_units.get(line.product_id, 0) + line.quantity_in_units
            )

    orderable_product_ids = {
        product_id for product_id, quantity in available_units.items() if quantity > 0
    }
    ...
    if not allowed_product_ids:
        ...
        queryset = (
            Product.objects.filter(
                Q(active=True) | Q(id__in=existing_product_ids),
