...
def test_order_line_form_accepts_valid_line(apple):
    ...
    assert form.cleaned_data["unit"] == "boxes"
    assert form.cleaned_data["quantity_in_boxes"] == 10
...
def test_order_line_form_rejects_unusually_large_line(apple):
    form = OrderLineForm(
        data={
            "product": str(apple.pk),
            "unit": "boxes",
            "quantity": str(MAX_BOXES_PER_PRODUCT_PER_ORDER + 1),
        },
        product_queryset=type(apple).objects.filter(pk=apple.pk),
        available_boxes_by_product_id={
            apple.id: 1000,
...
def test_build_product_choice_context_includes_only_orderable_products(
    ...
    assert context.available_boxes_by_product_id[apple.id] == 150
    assert context.available_boxes_by_product_id[banana.id] == 80
...
def test_build_product_choice_context_includes_existing_order_product_even_if_inactive(
    ...
    assert context.available_boxes_by_product_id[inactive_product.id] == 1
